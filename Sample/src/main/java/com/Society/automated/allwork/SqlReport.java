...
/**
 * Servlet implementation class SqlReport
 * 
 * THIS CLASS IS RESPONSIBLE FOR GENERATING AVAILABLE PAYMENT
 * 
 */
public class SqlReport extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	final static Logger logger=LoggerFactory.getLogger(SqlReport.class);
      
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String sql=request.getParameter("sql");
		System.out.println("SQL "+sql);
		SqlReportDao sqlreportdao=new SqlReportDao();
		String name=sqlreportdao.createReport(sql);
...
//                    Integer.parseInt(PROXY_PORT), fileURL  );
		...
		bos= new BufferedOutputStream(out);
		    ...
		    logger.info("All Payment Report {} Generated",name);
		}
